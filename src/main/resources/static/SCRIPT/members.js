$(document).ready(function () {
	...
	$('.searchBtn').click(function () {
		var btn = this;
		var searchValue = $(btn).siblings('INPUT[name="searchValue"]').val().replace(/^0/g, '');
		console.log(searchValue);
		var searchGender = $(btn).siblings('INPUT[name="searchGender"]').val();
		$.post(
			...
			function (data) {
				if (searchGender === 'true') {
					data.forEach(function (member) {
						console.log('male', member.nickname + member.login)
					});
					$('.maleMembers').empty();
				} else {
					data.forEach(function (member) {
						console.log('female', member.nickname + member.login)
					});
					$('.femaleMembers').empty();
				}
			},
	...
	$('INPUT[name="searchValue"]').keyup(function () {
		let input = this;
		var searchValue = $(input).val().replace(/^0/g, '');
		console.log(searchValue);
		var searchGender = $(input).siblings('INPUT[name="searchGender"]').val();
