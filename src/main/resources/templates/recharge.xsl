	...
	<xsl:template match="document">
		...
		<BODY>
			...
			<DIV class="container py-6 py-md-7">
				...
				<DIV class="text-center text-primary text-sm mx-auto col-11 col-md-6 pt-3 my-3" style="border-top: 2px dashed #CE796B;">
					<DIV>
				...
				<DIV class="text-center text-dark text-bold border-radius-xl mx-auto col-11 col-md-6 p-1 my-3" style="border: 1px solid #164353;">
					<SPAN>您剩餘的ME點：</SPAN>
					<SPAN>
					...
					<xsl:for-each select="plan">
						<DIV class="col-11 col-md-6 card mb-3 mx-2">
							<xsl:if test="position() = 1">
								<xsl:attribute name="style">background: #3d7386;</xsl:attribute>
							</xsl:if>
							<xsl:if test="position() = 2">
								<xsl:attribute name="style">background: #255668;</xsl:attribute>
							</xsl:if>
							<xsl:if test="position() = 3">
								<xsl:attribute name="style">background: #164353;</xsl:attribute>
							</xsl:if>
				...
				<DIV class="text-center text-dark text-xs border-radius-xl mx-auto col-11 col-md-6 p-1" style="border: 1px solid #164353;">
					<DIV>本筆款項將在信用卡帳單</DIV>
